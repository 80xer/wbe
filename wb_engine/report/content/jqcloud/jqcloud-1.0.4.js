  ...
  $.fn.jQCloud = function(word_array, options) {
    ...
    var drawWordCloud = function() {
      ...
      var drawOneWord = function(index, word) {
        ...
        // weight 구하는 곳인데
        // 개수나 너무 작을 경우에는 전체적인 크기를 키워주자        
        if (word_array.length < 50){
        	if (word_array[0].weight > word_array[word_array.length - 1].weight) {
                // Linearly map the original weight to a discrete scale from 1 to 10
                weight = Math.round( ((word.weight - word_array[word_array.length - 1].weight) /
                                     (word_array[0].weight - word_array[word_array.length - 1].weight) * 9.0) * 0.3 + 6.3) + 1;
           }
        }
        else if (word_array.length < 100){
        	if (word_array[0].weight > word_array[word_array.length - 1].weight) {
                // Linearly map the original weight to a discrete scale from 1 to 10
                weight = Math.round( ((word.weight - word_array[word_array.length - 1].weight) /
                                     (word_array[0].weight - word_array[word_array.length - 1].weight) * 9.0) * 0.5 + 4.5) + 1;
           }
        }
        else{
        	if (word_array[0].weight > word_array[word_array.length - 1].weight) {
